...
if isfield(model.const,'R0')
    ...
    psi = getParam('const','psi');
    kD = getParam('const','kD');
    Rc = getParam('const','Rc');
    Rf_p = getParam('pos','Rf');
    kappa_p = getParam('pos','kappa');
    sigma_p = getParam('pos','sigma');
    kappa_s = getParam('sep','kappa');
    kappa_d = getParam('DL','kappa');
    k0_n = getParam('neg','k0');
    Rf_n = getParam('neg','Rf');
    Rct_n = 1/f/k0_n;
    R0 = (Rf_p+Rct_n+Rf_n+Rc) + 1/sigma_p/3 + ...
         (1-kD/psi)*(1/kappa_p/3 + 1/kappa_s + 1/kappa_d);
end
