...
function ROMout = outBlend_phie(simData,ROM,varargin)
  ...
  if nargin >= 6, SOCtarget = varargin{4}; end

  if isempty(SOCtarget)
        mode = '';
  else
    if simData.SOC0/100 > SOCtarget
      mode = 'dchg';
    else 
      mode = 'chg';
    end
  ...
  cellState.cellSOC = simData.SOC0/100;
  ...
  for k = 0:length(ik)-1
    ...
    if (cellState.cellSOC >= SOCtarget) && strcmp(mode,'chg')
         ik(k+1) = 0;
    end
    if (cellState.cellSOC <= SOCtarget) && strcmp(mode,'dchg')
         ik(k+1) = 0;
    end
...
%     ROMout.negPhise(k+1,:) = yk(ROM.ind.negPhise)  + UocpnAvg;
    ...
    [R0, R123] = getR(Iapp);
    ROMout.Phie(k+1,1)     = yk(ROM.ind.negPhie) - (R0)*Iapp; 
    ROMout.Phie(k+1,2:end) = yk(ROM.ind.Phie) - yk(ROM.ind.negPhise0) - (R123)*Iapp; 
    ROMout.negPhise0(k+1)  = yk(ROM.ind.negPhise0) - (R0)*Iapp;
...
%     ROMout.posEta3(k+1) = ROMout.posPhise(k+1,end) - Uocpp3 - Rfp*ROMout.posIfdl3(k+1);
    
% ROMout.Vcell(k+1) = ROMout.posEta3(k+1) ...
%         + ROMout.Phie(k+1,end) + Uocpp3 ...
%         + Rfp*ROMout.posIfdl3(k+1) ;
    ...
    newCellState.cellSOC = ROMout.cellSOC(k+1);
    ROMout.i0(k+1) = i0p;
  end
...
%     ROMout.xLocs.Idl     = [tfLocs(ROM.ind.posIdl)];
  ...
  function shortWarn(msg)
    ...
    function i0 = computei0(k0,T,f)
          if length(k0) > 1 % Then, this is an MSMR-kinetics model
                Uref  = cellData.function.pos.Uocp(ROMout.posThetass3(k+1),T);
                X     = cellData.function.pos.X(ROMout.posSOC(k+1),T);
                U0    = cellData.function.pos.U0(ROMout.posSOC(k+1),T);
                W     = cellData.function.pos.omega(ROMout.posSOC(k+1),T);
                alpha = cellData.function.pos.alpha(ROMout.posSOC(k+1),T);
            ...
            % Compute fractions xj and i0 of each gallary (vectors)
                ...
                i0 = k0*sqrt(ROMout.Thetae(k+1,end)*...
                     (1-ROMout.posThetass3(k+1))*ROMout.posThetass3(k+1)); 
          end  
    end

    function [R0, R123] = getR(Iapp)
        if Iapp<0
            R0 = -5.667*(Iapp)^3 - 2.58*(Iapp)^2 - 0.677*Iapp - 0.0125;
            R1 = -7.81*Iapp^3 - 3.39*Iapp^2 - 0.825*Iapp - 0.0179;
            R2 = -9.14*Iapp^3 - 3.728*Iapp^2 - 0.972*Iapp - 0.0206;
            R3 = -9.128*Iapp^3 - 3.612*Iapp^2 - 0.8547*Iapp - 0.021;
            R123 = [R1; R2; R3];
        elseif Iapp>0
            R0 = 0.00825.*(Iapp).^3 - 0.279.*Iapp.^2 + 0.36.*Iapp - 0.00227;
            R1 = 0.261*Iapp + 0.007907;
            R2 = 0.3121*Iapp + 0.01049;
            R3 = -2.47*Iapp.^3 + 0.0341*Iapp.^2 + 0.248*Iapp + 0.000811;
            R123 = [-R1; -R2; -R3];
            R0 = -R0;
        else 
            R0 = 0;   R123 = 0;
