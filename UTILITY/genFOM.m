...
function genData = genFOM(cellModel,varargin)
  ...
  parser = inputParser;
  parser.addRequired('cellModel',@(x)isscalar(x)&&isstruct(x)&&isfield(x,'function'));
  ...
  function createModel
    ...
    FOM.mesh('mesh1d').feature('E1').feature('D1').set('elemcount','20');   % Changed 03/12/2022   Previous value: 20
  ...
  function setVariableValues
    ...
    FOM.variable('varsNeg1d').set('kappa', 'kappaDlFN(theta_e,T)');
    FOM.variable('varsNeg1d').set('qe', 'qeDlFN(0.5,T)');
