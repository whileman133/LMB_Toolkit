...
%     ROMout.negPhise(k+1,:) = yk(ROM.ind.negPhise)  + UocpnAvg;
    ...
    ROMout.Phie(k+1,2:end) = yk(ROM.ind.Phie) - yk(ROM.ind.negPhise0); 
    ...
    ROMout.Vcell(k+1) = ROMout.posEta3(k+1) + ROMout.Phie(k+1,1)...
...
%                 Uref  = cellData.function.pos.Uocp(ROMout.posSOC(k+1),T);
            ...
            % Compute fractions xj and i0 of each gallery (vectors)
                x    = X./(1+exp(f*(Uref-U0)./W)); % sum(xjn) = thetan
                i0   = sum(k0.*((x).^(W.*alpha)).*((X-x).^(W.*(1-alpha))).*ROMout.Thetae(k+1,end).^(1-alpha)); 
%                 i0   = sum(k0.*sqrt(((x).^(W)).*((X-x).^(W)).*ROMout.Thetae(k+1,end)));
