...
parser.addParameter('ConsolidateLayers','off', ...
    @(x)any(strcmpi(x,{'off','seponly','combine'})));
...
layerMode = parser.Results.ConsolidateLayers;
...
p.qes         = getParam('sep','qe');
p.kappad      = getParam('DL','kappa');
p.qed         = getParam('DL','qe');
...
if isParam('DL','nE')
    p.nEdl = getParam('DL','nE');
else
    p.nEdl = 1;
end
if isParam('sep','nE')
    p.nEs = getParam('sep','nE');
else
    p.nEs = 1;
end
if strcmpi(layerMode,'combine')
    p.qes = p.qes + p.qed;
    p.kappas = (p.kappad*p.kappas)/(p.kappad+p.kappas);
    p.nEs = mean([p.nEdl p.nEs]);
    p.kappad = Inf;
    p.qed = 0;
end
...
if isParam('pos','X') && isParam('pos','U0') && isParam('pos','omega')
    % MSMR model.
    electrode = MSMR(getReg('pos'),'pos','sortParams',false);
...
if calc11 || calc22
    data.h11 = tfLMB11(p,S,layerMode);
end
if calc22
    data.h22 = tfLMB22(data.h11,layerMode);
end
data.param = p;
    ...
    function fcn = splitFactory()
        ...
        function [Z, bins] = split2(X)
            ...
            bins.d = false(size(X));
            bins.s = 0<X&X<1;
            bins.p = 1<=X&X<=2;  % Some quantities not defined in s, so ensure x=2 is in p!
            Z.n = X(bins.n);
            Z.d = [];
            Z.s = X(bins.s);
            Z.p = 2-X(bins.p);
...
function data = tfLMB11(p,S,layerMode)
    ns = length(S);
