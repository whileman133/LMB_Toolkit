...
%       semilogx(W,20*log10(magTrue(1:2,:)),'linewidth',1.5); hold on
      set(gca,'colororderindex',1);       
      semilogx(W,20*log10(abs(magEst)),'x'); hold on;
%       semilogx(W,20*log10(abs(magEst(1:2,:))),'x'); hold on;
...
%       semilogx(W,phaseTrue(1:2,:),'linewidth',1.5); hold on
      set(gca,'colororderindex',1);       
      semilogx(W,phaseEst,'x'); hold on;
%        semilogx(W,phaseEst(1:2,:),'x'); hold on;
      semilogx([pi/Ts pi/Ts],ylim,'k--');
      xlim([min(W) 1.1*max(W)]); grid on
      xlabel('Analog frequency (rad/s)'); ylabel('Phase (deg)');
      title(sprintf('Phase plots: Line = truth; x = est.; SOC = %d%%',100*SOC));
      drawnow
      save('frequency_response.mat','W','Ts','phaseEst','phaseTrue','magTrue','magEst','SOC','xraData');
